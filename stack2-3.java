...
class Main{
    // リストを受け取り、単調増加している部分リストを返す関数を実装します。
    // リストの途中で単調減少する部分が出現したら、部分リストをリセットします。
    public static int[] consecutiveWalk(int[] arr){
        Stack stack = new Stack();
        stack.push(arr[0]);
        for(int i = 1; i < arr.length; i++){
            // スタックの上にある要素より、arr[i]が小さい場合、スタックをリセットします。
            
            if(stack.peek() >= arr[i]){
                // スタックがnullになるまで繰り返されます。
                while(stack.pop() != null);
            }
            // スタックにプッシュします。スタックは常に単調増加になっています。
            stack.push(arr[i]);
        }
    ...
    public static void main(String[] args){
        ...
        System.out.println();
        Stack.printList(consecutiveWalk(new int[]{1,2,3})); 
        System.out.println();
        Stack.printList(consecutiveWalk(new int[]{4,5,4,2,4,3646,34,64})); 
        System.out.println();
        Stack.printList(consecutiveWalk(new int[]{4,5,4,2,4,36,46})); 
    }
}
